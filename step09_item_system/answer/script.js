...
function usePotion() {
    ...
    // 回復アニメーション
    playerHPContainer.classList.add('heal-flash');
    setTimeout(() => {
        playerHPContainer.classList.remove('heal-flash');
    }, 500);
    ...
    setTimeout(() => {
        ...
        // 対戦エリアの背景色（ポーション使用は特別な色）
        battleArea.style.background = 'linear-gradient(135deg, #e8f5e9 0%, #fff9c4 100%)';

        // 結果表示
        resultDiv.textContent = `ポーション使用！+${actualHeal}HP回復、敵の攻撃で${damage}ダメージ`;
        resultDiv.style.color = '#4CAF50';
        resultDiv.style.backgroundColor = '#e8f5e9';

        // ログに記録
        const logEntry = document.createElement('div');
        logEntry.className = 'log-entry potion-used';
        logEntry.textContent = `Round ${roundCount}: ポーション使用 (+${actualHeal}HP、-${damage}HP)`;
        battleLogContainer.insertBefore(logEntry, battleLogContainer.firstChild);
...
function checkEnemyDefeated() {
    if (enemyHP <= 0) {
        ...
        if (actualHeal > 0) {
            ...
            healLog.className = 'log-entry heal';
            healLog.textContent = `撃破ボーナス: +${actualHeal}HP回復！`;
            battleLogContainer.insertBefore(healLog, battleLogContainer.firstChild);
        }
        ...
        if (currentEnemyIndex < enemies.length) {
            // 次の敵がいる
            disableButtons();
            resultDiv.textContent = `${enemies[currentEnemyIndex - 1].name}を倒した！`;
            if (actualHeal > 0) {
                resultDiv.textContent += ` +${actualHeal}HP回復！`;
            }
            resultDiv.textContent += ' 次の敵が現れる...';
...
function enableButtons() {
    ...
    updatePotionDisplay(); // ポーションの有効/無効はポーション数に依存
}
